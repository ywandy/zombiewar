...
func _ready() -> void:
	...
	var socket := player.find_child("WeaponHandSocket", true, false) as Node3D
	if socket != null:
		socket_marker.visible = true
		socket_marker.top_level = true
		socket_marker.global_transform = socket.global_transform
	else:
		status_label.text = "WeaponHandSocket not found"
	_update_status()
...
func _update_status() -> void:
	...
	status_label.text = "Generated Player Binding Test\nMove: WASD / Arrow Keys | Switch: 1-5\nWeapons: %s\nCurrent: %s\nVisual: %s" % [
